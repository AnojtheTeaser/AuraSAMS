...
//    loadAllSubjectID();
    ...
    private void printOnAction(ActionEvent event) {
        ...
        // this method generated by using AI
        
        
          if (reportTableList.isEmpty()) {
        new Alert(Alert.AlertType.WARNING, "No data available in the table to print!").show();
        return;
    }
    ...
    java.io.File file = fileChooser.showSaveDialog(print.getScene().getWindow());
    ...
    if (file != null) {
        ...
        try {
            ...
            com.itextpdf.text.Paragraph title = new com.itextpdf.text.Paragraph("AuraSAMS - Attendance Report\n\n");
            ...
            String[] headers = {"Student ID", "Name", "Course Name", "Subject", "Date", "Status"};
            for (String headerText : headers) {
                com.itextpdf.text.pdf.PdfPCell cell = new com.itextpdf.text.pdf.PdfPCell(new com.itextpdf.text.Paragraph(headerText));
            ...
            for (ReportDTO dto : reportTableList) {
                pdfTable.addCell(dto.getStuID());
                pdfTable.addCell(dto.getStuName());
                pdfTable.addCell(dto.getCourseName());
                pdfTable.addCell(dto.getSubjectName());
                pdfTable.addCell(dto.getDate());
                pdfTable.addCell(dto.getStatus());
            }
            ...
            document.close();
            new Alert(Alert.AlertType.INFORMATION, "PDF Report successfully saved to:\n" + file.getAbsolutePath()).show();

        } catch (Exception e) {
            e.printStackTrace();
            new Alert(Alert.AlertType.ERROR, "An error occurred while saving the PDF!").show();
        }
    }
